...
export function PlayerCountSelector({ onSelect }: PlayerCountSelectorProps) {
  ...
  return (
    <div className="relative min-h-dvh flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      >
        <motion.div
          className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-[#667eea]/20 to-[#764ba2]/20 border border-white/10 mb-4"
        >
        ...
        <p className="text-white/50 text-sm">How many people are playing?</p>
      </motion.div>
          ...
          {PLAYER_COUNTS.map((count, i) => (
            <motion.button
              ...
              className={`aspect-square rounded-xl font-black text-lg sm:text-xl transition-all duration-200 border-2 ${
                selected === count
                  ? "bg-gradient-to-br from-[#667eea] to-[#764ba2] border-white/30 shadow-lg shadow-[#667eea]/40 scale-105"
                  : "bg-white/[0.04] border-white/[0.06] hover:border-white/20 hover:bg-white/[0.08] text-white/80 hover:text-white"
              }`}
            >
              {count}
            </motion.button>
      ...
      <motion.p
        ...
        className="relative z-10 mt-6 text-xs text-white/20"
      >
        Choose wisely — the spy is among you
      </motion.p>
    </div>
  );
}
